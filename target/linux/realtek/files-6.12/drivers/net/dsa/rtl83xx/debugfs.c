...
void rtl838x_dbgfs_init(struct rtl838x_switch_priv *priv)
{
	...
	/* trk_mbr_ctr() and file names use the one-based DSA LAG ID. */
	for (int lag_id = 1; lag_id <= priv->ds->num_lag_ids; lag_id++) {
		snprintf(lag_name, sizeof(lag_name), "lag.%02d", lag_id);
		if (priv->family_id == RTL8380_FAMILY_ID)
