...
static int rtl930x_l3_setup(struct rtl838x_switch_priv *priv)
{
	...
	/* The IPv6 entry footprint spans three slots */
	set_bit(RTL930X_ROUTE_IDX_CATCHALL_IP6, priv->route_use_bm);
	set_bit(RTL930X_ROUTE_IDX_CATCHALL_IP6 + 1, priv->route_use_bm);
	set_bit(RTL930X_ROUTE_IDX_CATCHALL_IP6 + 2, priv->route_use_bm);
			  ...
			  u32 min_pages, u32 max_pages, u8 probability)
{
	...
	/* See RTL930X_FC_EGR_DROP_REF_RXCNGST: SWRED only sees the egress
	 * queue with the congestion reference cleared.
	 */
	sw_w32_mask(RTL930X_FC_EGR_DROP_REF_RXCNGST, 0,
...
void rtl930x_qos_swred_disable(struct rtl838x_switch_priv *priv, int port)
{
	...
	sw_w32_mask(RTL930X_FC_EGR_DROP_REF_RXCNGST,
		    RTL930X_FC_EGR_DROP_REF_RXCNGST,
		    RTL930X_FC_PORT_EGR_DROP_CTRL(port));
	mutex_unlock(&priv->reg_mutex);
}
